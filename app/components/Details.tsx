import { cn } from "~/lib/utils";
import {
  Accordion,
  AccordionContent,
  AccordionHeader,
  AccordionItem,
} from "./Accordion";

const ScoreBadge = ({ score }: { score: number }) => {
  return (
      <div
          className={cn(
              "flex flex-row gap-1 items-center px-2 py-0.5 border-2 border-black",
              score > 69
                  ? "bg-green-300"
                  : score > 39
                      ? "bg-yellow-300"
                      : "bg-red-300"
          )}
      >
        <img
            src={score > 69 ? "/icons/check.svg" : "/icons/warning.svg"}
            alt="score"
            className="size-4"
        />
        <p
            className={cn(
                "text-sm font-bold font-mono text-black"
            )}
        >
          {score}/100
        </p>
      </div>
  );
};

const CategoryHeader = ({
                          title,
                          categoryScore,
                        }: {
  title: string;
  categoryScore: number;
}) => {
  return (
      <div className="flex flex-row gap-4 items-center py-2">
        <p className="text-2xl font-semibold font-mono">{title}</p>
        <ScoreBadge score={categoryScore} />
      </div>
  );
};

const CategoryContent = ({
                           tips,
                         }: {
  tips: { type: "good" | "improve"; tip: string; explanation: string }[];
}) => {
  return (
      <div className="flex flex-col gap-4 items-center w-full">
        <div className="bg-gray-100 w-full border-2 border-black rounded-lg px-5 py-4 grid grid-cols-2 gap-4">
          {tips.map((tip, index) => (
              <div className="flex flex-row gap-2 items-center" key={index}>
                <img
                    src={
                      tip.type === "good" ? "/icons/check.svg" : "/icons/warning.svg"
                    }
                    alt="score"
                    className="size-5"
                />
                <p className="text-xl text-gray-700 font-mono">{tip.tip}</p>
              </div>
          ))}
        </div>
        <div className="flex flex-col gap-4 w-full">
          {tips.map((tip, index) => (
              <div
                  key={index + tip.tip}
                  className={cn(
                      "flex flex-col gap-2 rounded-lg p-4 border-2 border-black",
                      tip.type === "good"
                          ? "bg-green-200 text-green-900"
                          : "bg-yellow-200 text-yellow-900"
                  )}
              >
                <div className="flex flex-row gap-2 items-center">
                  <img
                      src={
                        tip.type === "good"
                            ? "/icons/check.svg"
                            : "/icons/warning.svg"
                      }
                      alt="score"
                      className="size-5"
                  />
                  <p className="text-xl font-semibold font-mono">{tip.tip}</p>
                </div>
                <p>{tip.explanation}</p>
              </div>
          ))}
        </div>
      </div>
  );
};

const Details = ({ feedback }: { feedback: Feedback }) => {
  return (
      <div className="flex flex-col gap-4 w-full">
        <Accordion>
          <AccordionItem id="tone-style">
            <AccordionHeader itemId="tone-style">
              <CategoryHeader
                  title="Tone & Style"
                  categoryScore={feedback.toneAndStyle.score}
              />
            </AccordionHeader>
            <AccordionContent itemId="tone-style">
              <CategoryContent tips={feedback.toneAndStyle.tips} />
            </AccordionContent>
          </AccordionItem>
          <AccordionItem id="content">
            <AccordionHeader itemId="content">
              <CategoryHeader
                  title="Content"
                  categoryScore={feedback.content.score}
              />
            </AccordionHeader>
            <AccordionContent itemId="content">
              <CategoryContent tips={feedback.content.tips} />
            </AccordionContent>
          </AccordionItem>
          <AccordionItem id="structure">
            <AccordionHeader itemId="structure">
              <CategoryHeader
                  title="Structure"
                  categoryScore={feedback.structure.score}
              />
            </AccordionHeader>
            <AccordionContent itemId="structure">
              <CategoryContent tips={feedback.structure.tips} />
            </AccordionContent>
          </AccordionItem>
          <AccordionItem id="skills">
            <AccordionHeader itemId="skills">
              <CategoryHeader
                  title="Skills"
                  categoryScore={feedback.skills.score}
              />
            </AccordionHeader>
            <AccordionContent itemId="skills">
              <CategoryContent tips={feedback.skills.tips} />
            </AccordionContent>
          </AccordionItem>
        </Accordion>
      </div>
  );
};

export default Details;

// import { cn } from "~/lib/utils";
// import {
//   Accordion,
//   AccordionContent,
//   AccordionHeader,
//   AccordionItem,
// } from "./Accordion";

// const ScoreBadge = ({ score }: { score: number }) => {
//   return (
//       <div
//           className={cn(
//               "flex flex-row gap-1 items-center px-2 py-0.5 border-2 border-black",
//               score > 69
//                   ? "bg-green-300"
//                   : score > 39
//                       ? "bg-yellow-300"
//                       : "bg-red-300"
//           )}
//       >
//         <img
//             src={score > 69 ? "/icons/check.svg" : "/icons/warning.svg"}
//             alt="score"
//             className="size-4"
//         />
//         <p
//             className={cn(
//                 "text-sm font-bold font-mono text-black"
//             )}
//         >
//           {score}/100
//         </p>
//       </div>
//   );
// };

// const CategoryHeader = ({
//                           title,
//                           categoryScore,
//                         }: {
//   title: string;
//   categoryScore: number;
// }) => {
//   return (
//       <div className="flex flex-row gap-4 items-center py-2">
//         <p className="text-2xl font-semibold font-mono">{title}</p>
//         <ScoreBadge score={categoryScore} />
//       </div>
//   );
// };

// const CategoryContent = ({
//                            tips,
//                          }: {
//   tips: { type: "good" | "improve"; tip: string; explanation: string }[];
// }) => {
//   return (
//       <div className="flex flex-col gap-4 items-center w-full">
//         <div className="bg-gray-100 w-full border-2 border-black rounded-lg px-5 py-4 grid grid-cols-1 md:grid-cols-2 gap-4">
//           {tips.map((tip, index) => (
//               <div className="flex flex-row gap-2 items-center" key={index}>
//                 <img
//                     src={
//                       tip.type === "good" ? "/icons/check.svg" : "/icons/warning.svg"
//                     }
//                     alt="score"
//                     className="size-5"
//                 />
//                 <p className="text-xl text-gray-700 font-mono">{tip.tip}</p>
//               </div>
//           ))}
//         </div>
//         <div className="flex flex-col gap-4 w-full max-h-80 overflow-y-auto">
//           {tips.map((tip, index) => (
//               <div
//                   key={index + tip.tip}
//                   className={cn(
//                       "flex flex-col gap-2 rounded-lg p-4 border-2 border-black",
//                       tip.type === "good"
//                           ? "bg-green-200 text-green-900"
//                           : "bg-yellow-200 text-yellow-900"
//                   )}
//               >
//                 <div className="flex flex-row gap-2 items-center">
//                   <img
//                       src={
//                         tip.type === "good"
//                             ? "/icons/check.svg"
//                             : "/icons/warning.svg"
//                       }
//                       alt="score"
//                       className="size-5"
//                   />
//                   <p className="text-xl font-semibold font-mono">{tip.tip}</p>
//                 </div>
//                 <p className="whitespace-pre-wrap">{tip.explanation}</p>
//               </div>
//           ))}
//         </div>
//       </div>
//   );
// };

// const Details = ({ feedback }: { feedback: Feedback }) => {
//   return (
//       <div className="flex flex-col gap-4 w-full">
//         <Accordion>
//           <AccordionItem id="tone-style">
//             <AccordionHeader itemId="tone-style">
//               <CategoryHeader
//                   title="Tone & Style"
//                   categoryScore={feedback.toneAndStyle.score}
//               />
//             </AccordionHeader>
//             <AccordionContent itemId="tone-style">
//               <CategoryContent tips={feedback.toneAndStyle.tips} />
//             </AccordionContent>
//           </AccordionItem>
//           <AccordionItem id="content">
//             <AccordionHeader itemId="content">
//               <CategoryHeader
//                   title="Content"
//                   categoryScore={feedback.content.score}
//               />
//             </AccordionHeader>
//             <AccordionContent itemId="content">
//               <CategoryContent tips={feedback.content.tips} />
//             </AccordionContent>
//           </AccordionItem>
//           <AccordionItem id="structure">
//             <AccordionHeader itemId="structure">
//               <CategoryHeader
//                   title="Structure"
//                   categoryScore={feedback.structure.score}
//               />
//             </AccordionHeader>
//             <AccordionContent itemId="structure">
//               <CategoryContent tips={feedback.structure.tips} />
//             </AccordionContent>
//           </AccordionItem>
//           <AccordionItem id="skills">
//             <AccordionHeader itemId="skills">
//               <CategoryHeader
//                   title="Skills"
//                   categoryScore={feedback.skills.score}
//               />
//             </AccordionHeader>
//             <AccordionContent itemId="skills">
//               <CategoryContent tips={feedback.skills.tips} />
//             </AccordionContent>
//           </AccordionItem>
//         </Accordion>
//       </div>
//   );
// };

// export default Details;